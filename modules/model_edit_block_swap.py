...
import modules.custom_offloading_utils as custom_offloading_utils
...
class Step1XEdit(nn.Module):
    ...
    def forward(
        ...
        for block_index, block in enumerate(self.double_blocks):
            if self.blocks_to_swap:
                self.offloader_double.wait_for_block(block_index)
            
            img, txt = block(img=img, txt=txt, vec=vec, pe=pe)

            if self.blocks_to_swap:
                self.offloader_double.submit_move_blocks(self.double_blocks, block_index)
        ...
        for block_index, block in enumerate(self.single_blocks):
            if self.blocks_to_swap:
                self.offloader_single.wait_for_block(block_index)
                
            img = block(img, vec=vec, pe=pe)
            
            if self.blocks_to_swap:
                self.offloader_single.submit_move_blocks(self.single_blocks, block_index)
